 ...
 */
public class TensorFlowPoseDetectorQD implements Classifier {
    private static final String TAG = "TensorFlowPoseDetector";

    public static final int MP_INPUT_SIZE = 256;
    //qingdao
    ...
    private static int NUM_HEATMAP = 32;
    private static int NUM_KEYPOINTS = 14;
    private static int NUM_PAF = 26;
    private static int NUM_WATCHED = 50;
    ...
    private TensorFlowPoseDetectorQD() {
    }
    ...
    public static Classifier create(
        ...
        TensorFlowPoseDetectorQD c = new TensorFlowPoseDetectorQD();
        c.inputName = MP_INPUT_NAME;

        try {
            c.inferenceInterface = new TensorFlowInferenceInterface(assetManager, MP_MODEL_FILE);
        } catch (RuntimeException re) {
            Log.e(TAG, "CAUSE " + re.getCause().getMessage(), re);
        }
        // The shape of the output is [N, NUM_CLASSES], where N is the batch size.
        int heatMap = (NUM_HEATMAP * NUM_HEATMAP * NUM_KEYPOINTS);//qing da
        int pafMat = (NUM_HEATMAP * NUM_HEATMAP * NUM_PAF);
...
//    Log.i(TAG, "Output layer 2 size is " + numClasses1);
        ...
        c.inputSize = MP_INPUT_SIZE;

        // Pre-allocate buffers.
        c.outputNames = new String[]{MP_OUTPUT_L1, MP_OUTPUT_L2};
        c.intValues = new int[MP_INPUT_SIZE * MP_INPUT_SIZE];
        c.floatValues = new float[MP_INPUT_SIZE * MP_INPUT_SIZE * 3];
...
//    }
        ...
        if (logStats){
            ...
            r0.heat = debugOutput(outputHeatMap, new int[]{NUM_HEATMAP, NUM_HEATMAP, NUM_KEYPOINTS});
            Log.i(TAG, "heat bmp = " + r0.heat.getHeight() + "," + r0.heat.getWidth());
            r0.pose = debugOutput(outputPafMat, new int[]{NUM_HEATMAP, NUM_HEATMAP, NUM_PAF});
...
//        String ss = "";
//        for (int idx = 0; idx < yy.length; idx++) {
//            ss += yy[idx] + " ";
//        }
//        Log.i(TAG, "DEMO: " +  ss);
        return o;
    ...
    private float[] non_max_suppression(float[] img, int w, int h, int window_size, float threshold) {
        ...
        int hws = (int) Math.floor(window_size / 2);//TODO:jiang 滑动步长
        // maximum_filter_only
        for (int i = 0; i < w; i++) {
            for (int j = 0; j < h; j++) {
                ...
                // find the window max，找最大窗口的
                float wm = 0;
                //TODO:jiang, Math.min(w, i + window_size - hws)防止滑动出界，超过w(32)
    ...
    /** 返回A、B中的元素的笛卡尔积的元组, 依次取出a中的每1个元素，与b中的每1个元素，组成元组*/
    ...
    private List<Human> estimatePose(float[] heatMat, float[] pafMat) {
        ...
        //TODO:jiang 坐标轴滚动，将骨骼点移动前面，方便循环
        ...
        for (int i = 0; i < NUM_KEYPOINTS; i++) {
            ...
            dd = restByMin(dd); //TODO:jiang 对axis=1,找到最小的，减去最小的
        ...
        for (int i = 0; i < NUM_KEYPOINTS; i++) {  // each image
            for (int j = 0; j < w; j++) { // each row
                ...
                rd = restByMin(rd);//TODO:jiang 对axis=2,找到最小的，减去最小的
        ...
        ArrayList[] coords = new ArrayList[NUM_KEYPOINTS];
        ...
        for (int i = 0; i < NUM_KEYPOINTS; i++) {
            ...
            coords[i].addAll(findCoords(nms, w, _NMS_Threshold));//TODO:jiang, findCoords找nms后大于阈值的坐标
        ...
        for (int i = 0; i < CommonQD.CocoPairs.length; i++) {
            ...
            List<Connection> connections = estimate_pose_pair(coords,//TODO:jiang, estimate_pose_pair找连接关系
                    idx1, idx2, // (idx1, idx2)
        ...
        //connection_by_human = dict()//TODO, jiangjunhou, group_limbs_of_same_person()?
        ...
        //TODO, jiang, 参考pose_decode.py的方法group_limbs_of_same_person()
        while (true) {
...
//                }
                ...
                for (Connection[] prod : conProd) {
                    ...
                    if (inBothSets(c1.uPartIdx, c2.uPartIdx)) {//TODO, jiang, 在两个人中进行merge
                        is_merged = true;
        ...
        ////# reject by subset count//TODO, jiang, 根据最小子集数Min_Subset_Cnt（4）拒绝
        ...
        ////# reject by subset max score//TODO, jiang, 根据最小子集分值Min_Subset_Score（0.8f）拒绝
    ...
    private List<Connection> estimate_pose_pair(ArrayList<Coord>[] coords, int partIdx1, int partIdx2, float[] pafMatX, float[] pafMatY, int w) {
        ...
        for (int idx1 = 0; idx1 < peak_coord1.size(); idx1++) {
            ...
            for (int idx2 = 0; idx2 < peak_coord2.size(); idx2++) {
                ...
                //score, count = get_score(x1, y1, x2, y2, pafMatX, pafMatY)//TODO:jiang, 计算连接关系
...
//            if (partIdx1, partIdx2) in [(2, 3), (3, 4), (5, 6), (6, 7)]:
                //int[][] cc1 = {{2, 3}, {3, 4}, {5, 6}, {6, 7}};//TODO, jiang, 18个点的右胳膊，右手臂，左胳膊，左手臂
                int[][] cc1 = {{0, 1}, {1, 2}, {3, 4}, {4, 5}};//TODO, jiang, 什么作用？14个点的右胳膊，右手臂，左胳膊，左手臂
        ...
        for (Connection candidate : sortConnections(connection_temp)) {
            //# check not connected//TODO, jiangjunhou, 用过的不能再用，一个连接关系不能属于两个人
    ...
    private ScoreOutput get_score(float x1, float y1, float x2, float y2, float[] pafMatX, float[] pafMatY, int w) {
        int __num_inter = 10;//TODO, jiang, 点(x1, y1)到点(x2, y2)的坐标插值个数，选10个，取这10个pafmap计算score
        float __num_inter_f = (float) __num_inter;
        float dx = x2 - x1;
        float dy = y2 - y1;
        //normVec = math.sqrt(dx ** 2 + dy ** 2)
        float normVec = (float) Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2));//TODO:jiang, 单位向量
        ...
        //xs = (xs + 0.5).astype(np.int8)//TODO:jiang, 为什么+0.5，防止是0?
        ...
        //pafYs = np.zeros(__num_inter)//TODO, jiang, 初始化__num_inter=10个pafmap为0
        ...
        //for idx, (mx, my) in enumerate(zip(xs, ys))://TODO, jiang, 取这10个pafmap
        ...
        //local_scores = pafXs * vx + pafYs * vy//TODO, jiang, patmap与坐标进行点积
        float[] local_scores = vector_sum(vector_mul(pafXs, vx), vector_mul(pafYs, vy));
        //thidxs = local_scores > Inter_Threashold
        boolean[] thidxs = vector_grater(local_scores, Inter_Threashold);//TODO, jiang, 找到score大于阈值的坐标
        //return sum(local_scores * thidxs), sum(thidxs)
        float score = sum(filter(local_scores, thidxs));//TODO, jiang, 根据score大于阈值的坐标过滤得到score值，并相加
